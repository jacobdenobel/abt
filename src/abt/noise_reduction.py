...
def noise_reduction(A, 
    ...
    **kwargs
    ...
    gMin = 1+(maxAttLin-1) /(1-1/(1+np.exp(-slopeFact*(snrFloor-snrSlope))))
    ...
    for iFrame in np.arange(nFrame):
        ...
        # compute gains
        SNR = V_s-V_n
        G[:,iFrame] = gMin+np.divide((1-gMin),1+np.exp(-slopeFact*(np.minimum(np.maximum(SNR,snrFloor),snrCeil)-snrSlope)))
    ...
    if enableContinuous:
        initState = {'V_s' : V_s,'V_n' : V_n,'Hold' : Hold,'HoldReady' : HoldReady,'HoldCount':HoldCount}
